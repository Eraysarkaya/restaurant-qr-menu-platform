...
export default async function AboutPage() {
  ...
  return <div>
    <section className="container-shell grid min-w-0 items-center gap-8 py-10 sm:py-14 lg:grid-cols-[minmax(0,.88fr)_minmax(0,1.12fr)] lg:gap-12 lg:py-16"><div className="min-w-0"><PageHeader eyebrow="Hakkımızda" title="İyi yemeğin başladığı küçük bir köşe." /><p className="mt-6 whitespace-pre-line break-words text-base leading-8 text-muted-foreground">{settings.longDescription}</p></div><div className="shadow-soft relative h-[300px] min-w-0 overflow-hidden rounded-2xl sm:h-[420px] lg:h-[520px]"><RestaurantImage src="/demo/restaurant-interior-v2.png" alt={`${settings.name} restoranının sıcak ve sade iç mekânı`} fill preload sizes="(max-width: 1024px) 100vw, 55vw" className="object-cover" /></div></section>
